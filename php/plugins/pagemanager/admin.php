...
if (isset($pagemanager)) {
	...
	case '':
        ...
        $server_name_port = $_SERVER['SERVER_NAME'];
        if( $_SERVER["SERVER_PORT"] != 80 ) $server_name_port .= ":" . $_SERVER["SERVER_PORT"];
		if (!headers_sent()) {
            // orig Yen adds the port number
		    //header('Location: http://'.$_SERVER['SERVER_NAME'].$sn
            header('Location: http://'.$server_name_port.$sn
