...
class XHFileBrowserView {
   ...
   {
    	$yenp=iconv("big-5","utf-8",$file);
        if(ini_get('magic_quotes_gpc')=="1"){
        $yenp=stripslashes($yenp);
        }
   }
   else
   {
	    $yenp=$file;
            ...
            $html .= '
                <li style="white-space:nowrap;" class="' . $class . '">
                    <form style="display: inline;" method="POST" action="" onsubmit="return confirmFileDelete(\'' . $this->translate('confirm_delete', $this->currentDirectory . $yenp) . '\');">
                        <input type="image" src="' . $this->browserPath . 'css/icons/delete.gif" alt="delete" title="delete file" />
                        <input type="hidden" name="deleteFile" />
                        <input type="hidden" name="file" value="' . $yenp . '" />
                    </form>
                    <form method="POST" style="display:none;" action="" id="rename_' . $i . '">
                        <input type="text" size="25" name="renameFile" value="' . $yenp . '" onmouseout="hideRenameForm(\'' . $i . '\');"/>
                        <input type="hidden" name="oldName" value="' . $yenp . '" />
                    </form>
                     <a style="position:relative" class="xhfbfile" href="#" onclick="return false" id="file_' . $i . '" ondblclick="showRenameForm(\'' . $i . '\', \'' . $this->translate('prompt_rename', $yenp) . '\');">' . $yenp;
            ...
            if (is_array(@getimagesize($this->basePath . $this->currentDirectory . $file))) {
                ...
                $html .= '<span style="position: relative;  z-index: 4; ">
                    ...
                    <img src="' . $this->basePath . $this->currentDirectory . $yenp . '" width="' . $width . 'px" height="' . $height . '" />' . tag('br') . $usage . '</span>';
            }
...
else
{
    $yenp=$file;
            ...
            $html .= '<a href="#" class="xhfbfile" onclick="window.setLink(\'' . $prefix . $yenp . '\',' . $is_image . '); return false;">' . $yenp;
            ...
            if (strpos($this->linkParams, 'type=images') !== false && getimagesize($dir . $file)) {
                ...
                $html .= '<span style="position: relative; z-index: 4;">
                    <span style="font-weight: normal; border: none;">' . $image[0] . ' x ' . $image[1] . ' px</span><br />
                    <img src="' . $this->basePath . $this->currentDirectory . $yenp . '" width="' . $width . 'px" height="' . $height . '" /></span>';
            }
